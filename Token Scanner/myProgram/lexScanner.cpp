...
//    in tokenVector, which is a vector of strings.
{
 ...
 //While it is not the end of thecharacter string stored in the statement array
 {
	...
	{	tokenBuffer += DOUBLE_QUOTE;  
		...
		tokenBuffer += DOUBLE_QUOTE; 
		//Copy the ending double quote into token buffer
	}
...
{
	if (isKEYWORD(token) == false && isSTRING_LITERAL(token) == false && !isNUMERICAL_LITERAL(token) && !isNUMERICAL_OP(token) && !isLOGICAL_OP(token) && !isASSIGNMENT_OP(token) && !isRELATIONAL_OP(token) && !isRIGHT_PARENTHESIS(token) && !isLEFT_PARENTHESIS(token))
	{
		return true;
	}
	return false;
	...
	perLineCategoryVector & categoryVector		)
{
	...
	{
		if (isNUMERICAL_LITERAL(tokenVector[x]) == true)
		{
			categoryVector.push_back(NUMERICAL_LITERAL);
		}
		else if (isSTRING_LITERAL(tokenVector[x]) == true)
		{
			categoryVector.push_back(STRING_LITERAL);
		}
		else if (isNUMERICAL_OP(tokenVector[x]) == true)
		{
			categoryVector.push_back(NUMERICAL_OP);
		}
		else if (isKEYWORD(tokenVector[x]) == true)
		{
			categoryVector.push_back(KEYWORD);
		}
		else if (isASSIGNMENT_OP(tokenVector[x]) == true)
		{
			categoryVector.push_back(ASSIGNMENT_OP);
		}
		else if (isRELATIONAL_OP(tokenVector[x]) == true)
		{
			categoryVector.push_back(RELATIONAL_OP);
		}
		else if (isCOLON(tokenVector[x]) == true)
		{
			categoryVector.push_back(COLON);
		}
		else if (isSEMICOLON(tokenVector[x]) == true)
		{
			categoryVector.push_back(SEMICOLON);
		}
		else if (isCOMMA(tokenVector[x]) == true)
		{
			categoryVector.push_back(COMMA);
		}
		else if (isRIGHT_CURLYBRACE(tokenVector[x]) == true)
		{
			categoryVector.push_back(RIGHT_CURLYBRACE);
		}
		else if (isLEFT_CURLYBRACE(tokenVector[x]) == true)
		{
			categoryVector.push_back(LEFT_CURLYBRACE);
		}
		else if (isRIGHT_PARENTHESIS(tokenVector[x]) == true)
		{
			categoryVector.push_back(RIGHT_PARENTHESIS);
		}
		else if (isLEFT_PARENTHESIS(tokenVector[x]) == true)
		{
			categoryVector.push_back(LEFT_PARENTHESIS);
		}
		else if (isLOGICAL_OP(tokenVector[x]) == true)
		{
			categoryVector.push_back(LOGICAL_OP);
		}
		else if (isCOMMENT(tokenVector[x]) == true)
		{
			categoryVector.push_back(COMMENT);
		}
		else if (isID_NAME(tokenVector[x]) == true)
		{
...
{
	tokenVectors.resize(linesOfStatements.size());
	int a = 0, b = 0, c = 0;
	for (int x = 0; x < tokenVectors.size(); x++, a++)
	{
		getPerLineTokenVectFromOneStringObject(linesOfStatements[a], tokenVectors[x]);
	}
//resize category
	categoryVectors.resize(tokenVectors.size());
//call getCategory
	for (int x = 0; x < tokenVectors.size(); x++)
	{
		getCategoryVectorFromTokenVector(tokenVectors[x], categoryVectors[x]);
	}
